...
class ApiServer():
    def __init__(self, uri_config : URIConfig, etcd_config: EtcdConfig, kafka_config : KafkaConfig):
        ...
        # --- 调试时使用 ---
        self.etcd.reset()
        self.kafka.delete_topics([self.kafka_config.SCHEDULER_TOPIC], operation_timeout=10)
    ...
    def add_scheduler(self):
        ...
        try:
            ...
            if not e.args[0].code() == 36: # 忽略“主题已存在”的错误
                raise
            else: print(f"[INFO]Topic '{topic}' already created.")
    ...
    def add_node(self, name : str):
        ...
        try:
            ...
            for topic, f in fs.items():
                f.result()
                print(f"[INFO]Topic '{topic}' created successfully.")
            self.kafka_producer.produce(kafka_topic, key='HEARTBEAT', value=json.dumps({}).encode('utf-8'))
        except KafkaException as e:
            if not e.args[0].code() == 36: # 忽略“主题已存在”的错误
                raise
    ...
    def get_pods(self, namespace : str):
        ...
        pods = self.etcd.get_prefix(self.etcd_config.PODS_KEY.format(namespace = namespace))
        
        # 格式化输出
        result = [{pod.name: pod.to_dict() if hasattr(pod, 'to_dict') else vars(pod)}
                  for pod in pods]
        return json.dumps(result)
    ...
    def add_pod(self, namespace : str, name : str):
        ...
        pod = self.etcd.get(self.etcd_config.POD_SPEC_KEY.format(namespace = namespace, name = name))
        if pod is not None:
            return json.dumps({'error': 'Pod name already exists'}), 409
        new_pod_config.status = POD_STATUS.CREATING
        self.etcd.put(self.etcd_config.POD_SPEC_KEY.format(namespace=namespace, name=name), new_pod_config)

        # 向scheduler推送消息
        try:
            self.kafka_producer.produce(self.kafka_config.SCHEDULER_TOPIC, value=pickle.dumps(new_pod_config))
    ...
    def bind_pod(self, namespace : str, name : str, node_name : str):
        ...
        self.etcd.put(self.etcd_config.POD_SPEC_KEY.format(namespace=namespace, name=name), pod)
    ...
    def update_pod_status(self, namespace : str, name : str):
        ...
        pod = self.etcd.get(self.etcd_config.POD_SPEC_KEY.format(namespace=namespace, name=name))
        if pod is None:
            return json.dumps({'message': f'Pod {namespace}:{name} is already deleted.'}), 404
        pod.status = status
        self.etcd.put(self.etcd_config.POD_SPEC_KEY.format(namespace=namespace, name=name), pod)
        print(f'[INFO]Pod {namespace}:{name} status change to {status}.')
        return json.dumps({'message': f'Pod {namespace}:{name} status change to {status}'}), 200
    ...
    def get_replica_set(self, namespace, name):
        ...
        key = self.etcd_config.REPLICA_SET_SPEC_KEY.format(namespace=namespace, name=name)
    ...
    def create_replica_set(self, namespace, name):
        ...
        try:
            ...
            pod_configs = self.etcd.get_prefix(self.etcd_config.PODS_KEY.format(namespace=namespace))
        ...
        try:
            ...
            if hasattr(target_rs, 'pod_instances') and target_rs.pod_instances:
                ...
                for pod in deleted_pods:
                    pod_key = self.etcd_config.POD_SPEC_KEY.format(namespace=namespace, name=pod.name)
    ...
    def create_hpa(self, namespace, name):
        ...
        try:
            ...
            if hpa_config.target_kind == 'ReplicaSet':
                rs_key = self.etcd_config.REPLICA_SET_SPEC_KEY.format(namespace=namespace, name=hpa_config.target_name)
    ...
    def _release_hpa_control(self, namespace, replica_set_name):
        ...
        rs_key = self.etcd_config.REPLICA_SET_SPEC_KEY.format(namespace=namespace, name=name)
        rs = self.etcd.get(rs_key)
